...
- `src/agent-teams/filesystem.ts`：读取全局 / 项目级 `aiyou-team.json`，归一化 Team 注册项，加载文件型 Team。
...
内置 Team 通过 `aiyou-team.json` 中的 `id` 引用：
...
全局配置缺失或安装修复时，aiyou-team 会默认加入内置 `coding-team`：

- `createDefaultConfig()` 默认返回 `teams: [{ id: "coding-team", enabled: true, priority: 0 }]`。
- packaged 模板 `templates/aiyou-team.json` 也默认启用 `coding-team`，其它模板 Team 默认 disabled。
...
文件型 Team 通过 `aiyou-team.json` 中的 `path` 引用：
...
aiyou-team 当前支持两个同构配置来源：

| 作用域 | 配置文件 | 路径解析基准 | source precedence |
| --- | --- | --- | --- |
| global | `<OpenCodeConfigRoot>/aiyou-team.json` | OpenCode config root | 低 |
| project | `<project-worktree>/.aiyou-team/aiyou-team.json` | `<project-worktree>/.aiyou-team` | 高 |
...
### 3.2 `aiyou-team.json` 当前 schema

当前 `aiyou-team.json` 的有效 Team entry 字段只有：
...
当前 `aiyou-team.json` 不支持 provider/model override、Team override、Agent override、extends、include 或 per-agent runtime patch。
...
不同工程通过各自 worktree 下的 `.aiyou-team/aiyou-team.json` 配置不同 Team。例如：
...
3. 同一 source 且 priority 相同，按 `aiyou-team.json` 中声明顺序。
...
- 当前 aiyou-team 不检查该 provider/model 是否在用户 OpenCode 环境中可用。

- 当前 aiyou-team 不根据 OpenCode 当前 UI-selected model 或用户默认 model 动态改写 Team model。
...
- 删除旧的 aiyou-team managed agent 中不再存在的 key。
- 插入新的 aiyou-team agent。
- 更新已有 aiyou-team managed agent。
- 遇到同名外部 agent 时跳过，避免覆盖非 aiyou-team agent。
...
用户可以在项目 `.aiyou-team/aiyou-team.json` 或全局 `aiyou-team.json` 中启用自己的文件型 Team，并设置更高优先级。但如果仍启用内置 `coding-team`，它仍会被投影，只是默认入口可能不是它。
...
### 6.3 当前不可行：只在 `aiyou-team.json` 中覆盖内置 Coding Team 的 model

当前 `aiyou-team.json` entry schema 不支持：
...
内置 Coding Team 的 `agentRuntime` 直接在 TypeScript 中硬编码。用户不能通过 `aiyou-team.json` 对内置 Team 的单个 Agent model 做 patch，只能替换整支 Team 或禁用它。
...
1. **配置 schema 层**：扩展 `aiyou-team.json`，支持全局 / 项目级 model 配置、Team override、Agent override。
...
| `aiyou-team.json` 读取与 Team source 归一化 | `src/agent-teams/filesystem.ts` |
