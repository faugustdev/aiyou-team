...
test("uninstallAiyouTeam detects cached package-layout installs in dry-run mode", async () => {
  ...
  const cachedPackageRoot = path.join(installRoot, "packages", "aiyou-team@latest", "node_modules", "aiyou-team");
...
test("uninstallAiyouTeam removes package workspace and legacy top-level residue", async () => {
  ...
  const packageWorkspaceRoot = path.join(installRoot, "packages", "aiyou-team@latest");
  const cachedPackageRoot = path.join(packageWorkspaceRoot, "node_modules", "aiyou-team");
